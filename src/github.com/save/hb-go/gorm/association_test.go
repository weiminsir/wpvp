...
func TestHasOne(t *testing.T) {
	...
	if DB.Model(&user).Association("CreditCard").Count() != 1 {
		t.Errorf("UserId's credit card count should be 1")
	}
	...
	if DB.Model(&user).Association("CreditCard").Count() != 0 {
		t.Errorf("UserId's credit card count should be 0 after Delete")
	}
	...
	if DB.Model(&user).Association("CreditCard").Count() != 0 {
		t.Errorf("UserId's credit card count should be 0 after Clear")
	}
	...
	if count := DB.Model(&user).Association("CreditCard").Count(); count != 1 {
		t.Errorf("UserId's credit card count should be 1 after Append, but got %v", count)
	}

	DB.Delete(&creditcard6)

	if count := DB.Model(&user).Association("CreditCard").Count(); count != 0 {
		t.Errorf("UserId's credit card count should be 0 after credit card deleted, but got %v", count)
	}

	if err := DB.Model(&user).Association("CreditCard").Find(&CreditCard{}).Error; err == nil {
		t.Errorf("UserId's creditcard is not findable after Delete")
	}

	if count := DB.Unscoped().Model(&user).Association("CreditCard").Count(); count != 1 {
		t.Errorf("UserId's credit card count should be 1 when query with Unscoped, but got %v", count)
	}

	if err := DB.Unscoped().Model(&user).Association("CreditCard").Find(&CreditCard{}).Error; err != nil {
		t.Errorf("UserId's creditcard should be findable when query with Unscoped, got %v", err)
	}
}
