...
func TestCreate(t *testing.T) {
	...
	if !DB.NewRecord(user) || !DB.NewRecord(&user) {
		t.Error("UserId should be new record before create")
	}
	...
	if DB.NewRecord(user) || DB.NewRecord(&user) {
		t.Error("UserId should not new record after save")
	}
	...
	if !reflect.DeepEqual(newUser.PasswordHash, []byte{'f', 'a', 'k', '4'}) {
		t.Errorf("UserId's PasswordHash should be saved ([]byte)")
	}

	if newUser.Age != 18 {
		t.Errorf("UserId's Age should be saved (int)")
	}

	if newUser.UserNum != Num(111) {
		t.Errorf("UserId's UserNum should be saved (custom type)")
	}
