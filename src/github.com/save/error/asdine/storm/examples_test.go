...
func ExampleDB_Begin() {
	...
	err = tx.One("UserId", 1, &account1)

	if err != nil {
		log.Fatal(err)
	}

	err = tx.One("UserId", 2, &account2)
	...
	err = db.One("UserId", 1, &account1Reloaded)

	if err != nil {
		log.Fatal(err)
	}

	err = db.One("UserId", 2, &account2Reloaded)
...
func ExampleDB_From() {
	...
	err = privateNotes.One("UserId", "work1", &workNote)

	// Not found: Wrong bucket.
	fmt.Println(err)

	err = workNotes.One("UserId", "work1", &workNote)

	if err != nil {
		log.Fatal(err)
	}

	err = privateNotes.One("UserId", "private1", &privateNote)
	...
	err = personalNotes.One("UserId", "personal1", &personalNote)
