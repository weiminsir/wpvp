...
type ServerImpl struct {
	...
	// debugger is the debugger control.
...
//  func (rcvr ReceiverType) Method(in InputType, cb control.RPCCallback)
func suitableMethods(rcvr interface{}, methods map[string]*methodType) {
	...
	if sname == "" {
		log.Printf("rpc.Register: no control name for type %s", typ)
		return
	}
	for m := 0; m < typ.NumMethod(); m++ {
		...
		synchronous := replyType.String() != "control.RPCCallback"
